...
int board_init (void)
{
	...
	gd->bd->bi_arch_number = MACH_TYPE_DIY2440;
...
#ifdef CONFIG_DIY2440_LED
void inline coloured_LED_init (void)
{
	struct s3c24x0_gpio * const gpio = s3c24x0_get_base_gpio ();
	S3C24X0_REG32 reg = readl (&gpio->GPFCON);
	reg = (reg & 0xFFFFB0FF) | 0x00001500;
	writel (reg, &gpio->GPFCON);

	reg = readl (&gpio->GPFDAT);
	reg = (reg | 0x70);
	writel (reg, &gpio->GPFDAT);
}

void inline red_LED_on (void)
{
	struct s3c24x0_gpio * const gpio = s3c24x0_get_base_gpio ();
	writel (readl(&gpio->GPFDAT) & ~(1<<4), &gpio->GPFDAT);
}
...
{
	struct s3c24x0_gpio * const gpio = s3c24x0_get_base_gpio ();
	writel (readl(&gpio->GPFDAT) & ~(1<<6), &gpio->GPFDAT);
}
