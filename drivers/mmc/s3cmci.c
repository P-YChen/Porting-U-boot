...
void get_sd_csd (struct sd_csd *csd, u_int32_t *pDat)
{
	if ((csd == NULL) && (pDat == NULL))
		return;
	switch ((pDat[0]&0xC0000000)>>30) {
		case CSD_VERSION_1:
			csd->version = 0;
			break;
		case CSD_VERSION_2:
	...
	if (csd->version == 0){
		csd->csd.csd_reg_v1.csd_taac = (pDat[0] & 0x00ff0000) >> 16;
		csd->csd.csd_reg_v1.csd_nsac = (pDat[0] & 0x0000ff00) >> 8;
		csd->csd.csd_reg_v1.csd_tran_speed = (pDat[0] & 0x000000ff);
		csd->csd.csd_reg_v1.csd_ccc = (pDat[1] & 0xfff00000) >> 20;
		csd->csd.csd_reg_v1.csd_read_bl_len = (pDat[1] & 0x000f0000) >> 16;
		csd->csd.csd_reg_v1.csd_c_size = (pDat[1] & 0x000003ff) << 2 + \
										 (pDat[2] & 0xC0000000) >> 30;
		csd->csd.csd_reg_v1.csd_vdd_r_curr_min = (pDat[2] & 0x38000000) >> 27;
		csd->csd.csd_reg_v1.csd_vdd_r_curr_max = (pDat[2] & 0x07000000) >> 24;
		csd->csd.csd_reg_v1.csd_vdd_w_curr_min = (pDat[2] & 0x00e00000) >> 21;
		csd->csd.csd_reg_v1.csd_vdd_w_curr_max = (pDat[2] & 0x001c0000) >> 18;
		csd->csd.csd_reg_v1.csd_c_size_mult = (pDat[2] & 0x00038000) >> 15;
		csd->csd.csd_reg_v1.csd_erase_blk_en = (pDat[2] & 0x00004000) >> 14;
		csd->csd.csd_reg_v1.csd_sector_size = (pDat[2] & 0x00003f80) >> 7;
		csd->csd.csd_reg_v1.csd_wp_grp_size = (pDat[2] & 0x0000007f);
		csd->csd.csd_reg_v1.csd_wp_grp_enable = (pDat[3] & 0x80000000) >> 31;
		csd->csd.csd_reg_v1.csd_r2w_factor = (pDat[3] & 0x1c000000) >> 26;
		csd->csd.csd_reg_v1.csd_write_bl_len = (pDat[3] & 0x03c00000) >> 22;
		csd->csd.csd_reg_v1.csd_write_bl_partial = (pDat[3] & 0x00200000) >> 21;
		csd->csd.csd_reg_v1.csd_file_format_grp = (pDat[3] & 0x00008000) >> 15;
		csd->csd.csd_reg_v1.csd_copy = (pDat[3] & 0x00004000) >> 14;
		if (pDat[3] & 0x00002000)
			csd->csd.csd_reg_v1.csd_write_protect |= CSD_REG_PERMANENT_WRITE_PROTECTION;
		
		if (pDat[3] & 0x00001000)
			csd->csd.csd_reg_v1.csd_write_protect |= CSD_REG_TEMPORARY_WRITE_PROTECTION;
		
		csd->csd.csd_reg_v1.csd_file_format = (pDat[3] & 0x00000c00) >> 10;
		csd->csd.csd_reg_v1.crc = (pDat[3] & 0x000000fe);
	}
	else if (csd->version == 1) {
		csd->csd.csd_reg_v2.csd_taac = (pDat[0] & 0x00ff0000) >> 16;
		csd->csd.csd_reg_v2.csd_nsac = (pDat[0] & 0x0000ff00) >> 8;
		csd->csd.csd_reg_v2.csd_tran_speed = (pDat[0] & 0x000000ff);
		csd->csd.csd_reg_v2.csd_ccc = (pDat[1] & 0xfff00000) >> 20;
		csd->csd.csd_reg_v2.csd_read_bl_len = (pDat[1] & 0x000f0000) >> 16;
		uint32_t temp1, temp2;
		temp1 = (pDat[1] & 0x000003f) << 16;
		temp2 = (pDat[2] & 0xffff0000) >> 16;
		csd->csd.csd_reg_v2.csd_c_size = temp1 + temp2;
		csd->csd.csd_reg_v2.csd_erase_blk_en = (pDat[2] & 0x00004000) >> 14;
		csd->csd.csd_reg_v2.csd_sector_size = (pDat[2] & 0x00003f80) >> 7;
		csd->csd.csd_reg_v2.csd_wp_grp_size = (pDat[2] & 0x0000007f);
		csd->csd.csd_reg_v2.csd_wp_grp_enable = (pDat[3] & 0x80000000) >> 31;
		csd->csd.csd_reg_v2.csd_r2w_factor = (pDat[3] & 0x1c000000) >> 26;
		csd->csd.csd_reg_v2.csd_write_bl_len = (pDat[3] & 0x03c00000) >> 22;
		csd->csd.csd_reg_v2.csd_write_bl_partial = (pDat[3] & 0x00200000) >> 21;
		csd->csd.csd_reg_v2.csd_file_format_grp = (pDat[3] & 0x00008000) >> 15;
		csd->csd.csd_reg_v2.csd_copy = (pDat[3] & 0x00004000) >> 14;
		if (pDat[3] & 0x00002000)
			csd->csd.csd_reg_v2.csd_write_protect |= CSD_REG_PERMANENT_WRITE_PROTECTION;
		
		if (pDat[3] & 0x00001000)
			csd->csd.csd_reg_v2.csd_write_protect |= CSD_REG_TEMPORARY_WRITE_PROTECTION;
		
		csd->csd.csd_reg_v2.csd_file_format = (pDat[3] & 0x00000c00) >> 10;
		csd->csd.csd_reg_v2.crc = (pDat[3] & 0x000000fe);
	}
	
}

static void print_sd_csd (const struct sd_csd *csd)
{
	if (csd->version == 0) {
		uint32_t MULT = 0x01 << (csd->csd.csd_reg_v1.csd_c_size_mult+2);
		uint32_t BLOCK_LEN = 0x01 << (csd->csd.csd_reg_v1.csd_read_bl_len);
		uint32_t BLOCKNR = (csd->csd.csd_reg_v1.csd_c_size + 1) * MULT;
		printf ("version = 0\n");
		printf ("memory capacity = %dG\n", BLOCK_LEN * BLOCKNR / 1024 / 1024 / 1024);
		printf ("crc = 0x%08x\n", csd->csd.csd_reg_v1.crc);
	}
	else if (csd->version == 1) {
		printf ("version = 1\n");
		printf ("memory capacity = %dG\n", (csd->csd.csd_reg_v2.csd_c_size+1)*512 / 1024 / 1024);
		printf ("crc = 0x%08x\n", csd->csd.csd_reg_v2.crc);
	}
	else {
		printf ("unknown sd card version.\n");
		return;
	}
}
